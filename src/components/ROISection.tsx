    ...
    <div className="mb-6">
      ...
      <div className="relative h-2 rounded-full bg-secondary">
        <div
          ...
          style={{ width: `${pct}%`, background: "linear-gradient(90deg, hsl(187 85% 55%), hsl(217 91% 55%))" }}
        />
        <input
          type="range"
          min={min}
        ...
        />
        <div
          className="absolute top-1/2 -translate-y-1/2 w-5 h-5 rounded-full border-2 border-primary shadow-cyan pointer-events-none"
          style={{
            left: `calc(${pct}% - 10px)`,
...
const StatCard = ({ label, value, sub, icon: Icon, color, delay }: StatCardProps) => {
  const colors = {
    cyan: { text: "text-primary", bg: "bg-primary/10", border: "border-primary/30" },
  ...
  return (
    <motion.div
      ...
      className={`card-premium p-6 border ${c.border}`}
    >
...
const ROISection = () => {
  ...
  const r = useCallback(() => {
    ...
    return {
      lostRevenue, timeWasted, totalLeakage, netGain,
      roi: Math.max(0, roi),
      breakevenJobs: Math.ceil((ANNUAL_INVESTMENT / (jobValue * cr)) * 10) / 10,
  ...
  const reset = () => { setJobValue(15000); setCloseRate(10); setMissedLeads(15); setAdminHours(40); setAdminRate(25); };

  const bars = [
    { label: "Current Annual Leakage",          value: fmt(r.totalLeakage), pct: r.leakagePct,    color: "from-red-600 to-red-400" },
    { label: "Flowlayer AI Year 1 Investment",   value: "$27,500",           pct: r.investmentPct, color: "from-blue-600 to-primary" },
    { label: "Net Annual Gain With Flowlayer AI",value: r.netGain > 0 ? fmt(r.netGain) : "$0", pct: r.gainPct, color: "from-emerald-600 to-emerald-400" },
  ];

  return (
    <section id="roi" className="py-24 lg:py-36 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        >
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Adjust the sliders to see exactly how much revenue you're losing — and how fast Flowlayer AI pays for itself.
          </p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-6">
          ...
          >
            <div className="flex items-center gap-2 mb-8">
              ...
              <h3 className="text-xs font-bold text-primary uppercase tracking-widest">Your Business Metrics</h3>
            </div>
            <SliderRow label="Average Job Value"           min={1000}  max={50000} step={500} value={jobValue}    display={`$${jobValue.toLocaleString()}`} onChange={setJobValue} />
            <SliderRow label="Close Rate"                  min={5}     max={80}    step={5}   value={closeRate}   display={`${closeRate}%`}                 onChange={setCloseRate} />
            <SliderRow label="Monthly Missed Leads"        min={1}     max={100}   step={1}   value={missedLeads} display={`${missedLeads}`}                onChange={setMissedLeads} />
            <SliderRow label="Monthly Admin Hours Wasted"  min={1}     max={160}   step={1}   value={adminHours}  display={`${adminHours} hrs`}             onChange={setAdminHours} />
            <SliderRow label="Admin Hourly Rate"           min={10}    max={100}   step={5}   value={adminRate}   display={`$${adminRate}/hr`}              onChange={setAdminRate} />
            <button onClick={reset} className="w-full mt-2 py-3 text-xs font-bold text-muted-foreground uppercase tracking-widest border border-border rounded-xl hover:border-primary/30 hover:text-primary transition-all">
              ↺ Reset to Defaults
            </button>
          </motion.div>
          ...
          >
            <div className="flex items-center gap-2 mb-8">
              <div className="w-1 h-5 rounded-full gradient-premium" />
              <h3 className="text-xs font-bold text-primary uppercase tracking-widest">Flowlayer AI Investment</h3>
            </div>
            {[
            ...
            <div className="mt-4 space-y-2">
              {["AI Voice Receptionist", "Roofing Lead Tracker", "Expense Tracker", "Full Setup Included"].map((item) => (
                <div key={item} className="flex justify-between items-center py-2.5 border-b border-border/50">
                  <span className="text-sm text-muted-foreground">{item}</span>
                  <span className="text-xs font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1 rounded-full">Included</span>
                </div>
              ))}
            </div>
        ...
        >
          <div className="flex items-center gap-2 mb-8">
            <div className="w-1 h-5 rounded-full gradient-premium" />
            <h3 className="text-xs font-bold text-primary uppercase tracking-widest">Financial Impact Comparison</h3>
          </div>
          {bars.map((bar) => (
        ...
        >
          ...
          <div className="relative">
            ...
            <p className="text-muted-foreground text-base max-w-xl mx-auto leading-relaxed">
              ...
              <strong className="text-foreground">{r.breakevenJobs} jobs per year</strong> covers your entire Flowlayer AI investment.
            </p>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="relative">
            <h3 className="text-3xl lg:text-4xl font-black text-white mb-4">Ready to Stop Bleeding Revenue?</h3>
            <p className="text-white/80 text-lg mb-10 max-w-xl mx-auto">
              Flowlayer AI sets up your full automation system in under 1 hour — all running on autopilot from day one.
            </p>
